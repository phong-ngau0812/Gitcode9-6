...
// <fileinfo name="Base\WH_OrderCollectionBase.cs">
//		<copyright>
//			All rights reserved.
//		</copyright>
//		<remarks>
...
/// </remarks>
public abstract class WH_OrderCollectionBase
...
{	
	...
	public WH_OrderCollectionBase(Database db) : base(db)
	{
		this.TABLENAME = "WH_Order";
	}

	public WH_OrderCollectionBase() : base()
	{
		this.TABLENAME = "WH_Order";     
	}
	...
	/// <summary>
	/// Gets an array of all records from the <c>WH_Order</c> table.
	/// </summary>
	/// <returns>An array of <see cref="WH_OrderRow"/> objects.</returns>
	public virtual WH_OrderRow[] GetAll()
	{
	...
	/// includes all records from the <c>WH_Order</c> table.
	/// </summary>
	...
	/// to retrieve all records from the <c>WH_Order</c> table.
	/// </summary>
	...
	protected virtual IDbCommand CreateGetAllCommand()
	{
		...
		try
		{
			return _db.CreateCommand("WH_Order_GetAll", CommandType.StoredProcedure);
		}
		catch (Exception ex)
        	{
		      	throw new Exceptions.DatabaseException(LOCATION, ex);
	...
	/// <summary>
	/// Gets the first <see cref="WH_OrderRow"/> objects that 
	/// match the search condition.
	/// </summary>
	/// <param name="whereSql">The SQL search condition. For example: 
	/// <c>"FirstName='Smith' AND Zip=75038"</c>.</param>
	/// <returns>An instance of <see cref="WH_OrderRow"/> or null reference 
	/// (Nothing in Visual Basic) if the object was not found.</returns>
	public WH_OrderRow GetRow(string whereSql)
	{
		string LOCATION = "GetRow(string whereSql)";
		try
        	{
			int totalRecordCount = -1;
			WH_OrderRow[] rows = GetAsArray(whereSql, null, 0, 1, ref totalRecordCount);
			if (0 == rows.Length)
				return null;
			return rows[0];
		}
	...
	/// <summary>
	/// Gets an array of <see cref="WH_OrderRow"/> objects that 
	...
	/// <returns>An array of <see cref="WH_OrderRow"/> objects.</returns>
	public WH_OrderRow[] GetAsArray(string whereSql, string orderBySql)
	{
	...
	/// <returns>An array of <see cref="WH_OrderRow"/> objects.</returns>
	public virtual WH_OrderRow[] GetAsArray(string whereSql, string orderBySql, int startIndex, int length, ref int totalRecordCount)
	{
		...
		try
		{
			...
			WH_OrderRow[] rows = MapRecords(reader);
			reader.Close();
	...
	protected virtual IDbCommand CreateGetCommand(string whereSql, string orderBySql)
	{
		...
		try
		{
			IDbCommand cmd = _db.CreateCommand("WH_Order_GetAdHoc", CommandType.StoredProcedure);
	...
	protected virtual IDbCommand CreateGetCommand(string whereSql, string orderBySql, int startIndex, int length)
	{
		...
		try
		{
			IDbCommand cmd = _db.CreateCommand("WH_Order_GetPageAdHoc", CommandType.StoredProcedure);
	...
	/// <summary>
	/// Gets <see cref="WH_OrderRow"/> by the primary key.
	/// </summary>
	/// <param name="order_ID">The <c>Order_ID</c> column value.</param>
	/// <returns>An instance of <see cref="WH_OrderRow"/> or null reference 
	/// (Nothing in Visual Basic) if the object was not found.</returns>
	public virtual WH_OrderRow GetByPrimaryKey(int order_ID)
	{
		string LOCATION = "GetByPrimaryKey(int order_ID)";
		try
        	{
			IDbCommand cmd = _db.CreateCommand("WH_Order_GetByPrimaryKey", CommandType.StoredProcedure);
			AddParameter(cmd, "Order_ID", order_ID);
			WH_OrderRow[] tempArray = MapRecords(cmd);
	...
	/// <summary>
	/// Adds a new record into the <c>WH_Order</c> table.
	/// </summary>
	/// <param name="value">The <see cref="WH_OrderRow"/> object to be inserted.</param>
	public virtual void Insert(WH_OrderRow value)
	{
		string LOCATION = "Insert(WH_OrderRow value)";
		try
		{
				IDbCommand cmd = _db.CreateCommand("WH_Order_Insert", CommandType.StoredProcedure);
					// Agency_ID
			if (value.IsAgency_IDNull)
				AddParameter(cmd, "Agency_ID", DBNull.Value);
			else
			...
			if (value.IsCodeNull )
				...
				AddParameter(cmd, "Code", value.Code);
						...
						value.Order_ID = Convert.ToInt32(cmd.ExecuteScalar());
			}
		catch(Exception ex)
		{
			throw new Exceptions.DatabaseException(LOCATION, ex);
	...
	/// <summary>
	/// Updates a record in the <c>WH_Order</c> table.
	/// </summary>
	/// <param name="value">The <see cref="WH_OrderRow"/>
	/// object used to update the table record.</param>
	/// <returns>true if the record was updated; otherwise, false.</returns>
	public virtual bool Update(WH_OrderRow value)
	{	
		string LOCATION = "Update(WH_OrderRow value)";
		try
		{
				IDbCommand cmd = _db.CreateCommand("WH_Order_Update", CommandType.StoredProcedure);
					// Agency_ID
			if (value.IsAgency_IDNull)
				AddParameter(cmd, "Agency_ID", DBNull.Value);
			else
        ...
        catch(Exception ex)
	{
		...
		/// <summary>
		/// Inserts or Updates a record in the <c>WH_Order</c> table.
		/// If a row with the specified primary key exists then it is updated
		/// otehrwise, a new row is added.
		/// </summary>
		/// <param name="value">The <see cref="WH_OrderRow"/>
		/// object used to save the table record.</param>
		public virtual void Save(WH_OrderRow value)
		{
			string LOCATION = "Save(WH_OrderRow value)";
			try
			{
					IDbCommand cmd =  _db.CreateCommand("WH_Order_Save",CommandType.StoredProcedure);
					// Agency_ID
				if (value.IsAgency_IDNull)
					AddParameter(cmd, "Agency_ID", DBNull.Value);
				else
					AddParameter(cmd, "Agency_ID", (object)(value.Agency_ID));
			...
			for(int i = rows.Count - 1; i >= 0; i--)
			{
				...
				switch(row.RowState)
				{
					...
					case DataRowState.Deleted:
						...
						try
						{
							DeleteByPrimaryKey((int)row["Order_ID"]);
						}
						finally
						{
							row.Delete();
	...
	/// <summary>
	/// Deletes the specified object from the <c>WH_Order</c> table.
	/// </summary>
	/// <param name="value">The <see cref="WH_OrderRow"/> object to delete.</param>
	/// <returns>true if the record was deleted; otherwise, false.</returns>
	public bool Delete(WH_OrderRow value)
	{
		return DeleteByPrimaryKey(value.Order_ID);
	}

	/// <summary>
	/// Deletes a record from the <c>WH_Order</c> table using
	/// the specified primary key.
	/// </summary>
	/// <param name="order_ID">The <c>Order_ID</c> column value.</param>
	/// <returns>true if the record was deleted; otherwise, false.</returns>
	public virtual bool DeleteByPrimaryKey(int order_ID)
	{
		string LOCATION = "DeleteByPrimaryKey(int order_ID)";
		try
		{
				IDbCommand cmd = _db.CreateCommand("WH_Order_DeleteByPrimaryKey", CommandType.StoredProcedure);
					AddParameter(cmd, "Order_ID", order_ID);
				return (0 < cmd.ExecuteNonQuery());
	}
        catch(Exception ex)
	...
	/// <summary>
	/// Deletes <c>WH_Order</c> records that match the specified criteria.
	/// </summary>
	...
	/// to delete <c>WH_Order</c> records that match the specified criteria.
	/// </summary>
	/// <param name="whereSql">The SQL search condition. 
	/// For example: <c>"FirstName='Smith' AND Zip=75038"</c>.</param>
	/// <returns>A reference to the <see cref="System.Data.IDbCommand"/> object.</returns>
	protected virtual IDbCommand CreateDeleteCommand(string whereSql)
	{
		...
		try
		{
			IDbCommand cmd = _db.CreateCommand("WH_Order_DeleteAdHoc", CommandType.StoredProcedure);
	...
	/// <summary>
	/// Deletes all records from the <c>WH_Order</c> table.
	/// </summary>
	/// <returns>The number of deleted records.</returns>
	public int DeleteAll()
	{
		...
		try
		{
				return _db.CreateCommand("WH_Order_DeleteAll", CommandType.StoredProcedure).ExecuteNonQuery();
			}
		catch(Exception ex)
		{
		      	throw new Exceptions.DatabaseException(LOCATION, ex);
	...
	/// <returns>An array of <see cref="WH_OrderRow"/> objects.</returns>
	protected WH_OrderRow[] MapRecords(IDbCommand command)
	{
	...
	/// <returns>An array of <see cref="WH_OrderRow"/> objects.</returns>
	protected WH_OrderRow[] MapRecords(IDataReader reader)
	{
		int totalRecordCount = -1;
	...
	/// of records in the reader object if 0 was passed into the method; otherwise it returns -1.</param>
	/// <returns>An array of <see cref="WH_OrderRow"/> objects.</returns>
	protected virtual WH_OrderRow[] MapRecords(IDataReader reader, int startIndex, int length, ref int totalRecordCount)
	{ 
		...
		while(reader.Read())
		{
			...
			if(ri > 0 && ri <= length)
			{
				WH_OrderRow record = new WH_OrderRow();
		...
		return (WH_OrderRow[])(recordList.ToArray(typeof(WH_OrderRow)));
	}
	...
	/// <summary>
	/// Converts <see cref="System.Data.DataRow"/> to <see cref="WH_OrderRow"/>.
	/// </summary>
	/// <param name="row">The <see cref="System.Data.DataRow"/> object to be mapped.</param>
	/// <returns>A reference to the <see cref="WH_OrderRow"/> object.</returns>
	protected virtual WH_OrderRow MapRow(DataRow row)
	{
		string LOCATION = "WH_OrderRow MapRow(DataRow row)";
		WH_OrderRow mappedObject = new WH_OrderRow();
	...
	/// <summary>
	/// Exist <see cref="WH_OrderRow"/> by the primary key.
	/// </summary>
	/// <param name="order_ID">The <c>Order_ID</c> column value.</param>
	/// <returns>An instance of <see cref="WH_OrderRow"/> or null reference 
	/// (Nothing in Visual Basic) if the object was not found.</returns>
	public virtual bool Exist(int order_ID)
	{
