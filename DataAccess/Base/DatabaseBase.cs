...
namespace Base
{
            ...
            : IDisposable
    {
        ...
        protected DatabaseBase(string DBName)
        {
            string LOCATION = "InitConnection(DBName)";
            try
            {
                if(DBName.ToUpper()=="HY")
                {
                    _connection = CreateHYConnection();
                    _connection.Open();
                }  
                else
                {
        ...
        public abstract IDbConnection CreateHYConnection();
